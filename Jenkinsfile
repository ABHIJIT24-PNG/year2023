pipeline {
    ...
    stages {
        ...
        stage(' Docker push to Docker Hub') {
           steps {
              script {
                 withCredentials([usernamePassword(credentialsId: 'dockerhubCred', usernameVariable: 'abhijit76', passwordVariable: '*abhijit76')]) {
                 sh "docker login -u ${abhijit76} -p ${*abhijit76}"
